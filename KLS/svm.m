...
% csvwrite(strcat('group'),Group);
% hold on;
% plot(testdata(:,1),testdata(:,2),'ro','MarkerSize',12);       % testing
% hold off
% struct('ClassNames',{{'1','-1'}},'ClassProbs',[1,2])
SVMModel = fitcsvm(traindata,group,'Prior',struct('ClassNames',{{'1','-1'}},'ClassProbs',[1,2]));
[label,score] = predict(SVMModel,testdata);
csvwrite(strcat('score'),score);
csvwrite(strcat('label'),label);
